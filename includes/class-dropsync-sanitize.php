 ...
 *
 * @package    Drop Sync
 * @subpackage Drop Sync/includes
 */

class Dropsync_Sanitize {
	...
	public function clean() {
		 ...
		 */
		do_action( 'scopeship_pre_sanitize', $sanitized );
		 ...
		 */
		do_action( 'scopeship_post_sanitize', $sanitized );
	...
	public function set_type( $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'dropsync' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'dropsync' ) );
